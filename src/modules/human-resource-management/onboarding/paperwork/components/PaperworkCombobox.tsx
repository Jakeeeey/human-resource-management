...
// Module-local combobox for the paperwork registry.
// COPIED from recruitment/mailing/components/MailCombobox.tsx (trigger truncate +
// v4 popover width + item truncate + title + Check toggle per QA.md §5.2) —
// never import the mailing module.

export interface PaperworkComboboxProps {
...
/**
 * Searchable combobox with truncated trigger/item labels.
 * @param options - Value/label pairs shown in the dropdown.
 * @param value - Currently selected value.
 * @param onValueChange - Called with the new value on select.
 * @param placeholder - Shown when nothing is selected.
 * @param disabled - Disables the trigger.
 * @param className - Extra trigger classes.
 * @returns The combobox trigger + popover.
 */
export function PaperworkCombobox({
    options,
    value,
    onValueChange,
