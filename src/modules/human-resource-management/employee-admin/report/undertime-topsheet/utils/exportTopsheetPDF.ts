...
export async function exportTopsheetPDF(
  ...
  const fileName = `Undertime_Topsheet_${exportType}_${new Date().toISOString().split("T")[0]}.pdf`;
    ...
    const doc = await PdfEngine.generateWithFrame(
      ...
      (doc, startY, config) => {
        ...
        if (exportType === "both" || exportType === "topsheet") {
          ...
          doc.text("UNDERTIME TOPSHEET", margins.left, currentY, { baseline: "top" });
          currentY += 8;
          ...
          autoTable(doc, {
            startY: currentY,
            head: [["Employee Name", "Department", "Total Approved UT"]],
            body: topsheetBody,
            margin: margins,
            theme: "striped",
            tableWidth: usableW,
        ...
        if (exportType === "both" || exportType === "reasons") {
          ...
          if (exportType === "both") {
            ...
            doc.setFontSize(14);
            doc.setFont("helvetica", "bold");
            doc.text("UNDERTIME REASONS", margins.left, currentY, { baseline: "top" });
            currentY += 8;
          ...
          data.forEach((userSum) => {
            ...
            autoTable(doc, {
              ...
              columnStyles: {
                0: { cellWidth: 30 },
                1: { cellWidth: 30 },
                2: { cellWidth: usableW - 60 }, // Remaining width for reason
              },
              didDrawPage: (dataInfo: { pageNumber: number }) => {
