...
import { fetchUndertimeTopsheet } from "../providers/fetchProvider";
import type { TopsheetUserSummary } from "../type";
import { toast } from "sonner";
import { format } from "date-fns";

export function useUndertimeTopsheet() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await fetchUndertimeTopsheet(startStr, endStr, departmentId);
      setData(response.data);
      if (response.departments) {
        setDepartments(response.departments);
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : "Failed to load undertime topsheet";
