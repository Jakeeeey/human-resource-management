...
export async function exportTopsheetPDF(
  ...
  const fileName = `Leave_Topsheet_${exportType}_${new Date().toISOString().split("T")[0]}.pdf`;
    ...
    const doc = await PdfEngine.generateWithFrame(
      ...
      (doc, startY, config) => {
        ...
        if (exportType === "both" || exportType === "topsheet") {
          ...
          doc.text("LEAVE TOPSHEET", margins.left, currentY, { baseline: "top" });
          currentY += 8;
          ...
          const topsheetBody = data.map((u) => [
            u.employee_name,
            u.department_name || "—",
            u.total_leave_days.toString(),
          ]);

          autoTable(doc, {
            startY: currentY,
            head: [["Employee Name", "Department", "Total Approved Leave Days"]],
            body: topsheetBody,
            margin: margins,
            theme: "striped",
            tableWidth: usableW,
        ...
        if (exportType === "both" || exportType === "reasons") {
          ...
          if (exportType === "both") {
            ...
            doc.setFontSize(14);
            doc.setFont("helvetica", "bold");
            doc.text("LEAVE REASONS", margins.left, currentY, { baseline: "top" });
            currentY += 8;
          ...
          data.forEach((userSum) => {
            ...
            const reasonsBody = userSum.requests.map((r) => [
              formatDateStr(r.leave_start),
              formatDateStr(r.leave_end),
            ...
            autoTable(doc, {
              startY: currentY,
              head: [["Start Date", "End Date", "Days", "Reason"]],
              body: reasonsBody,
              margin: margins,
              theme: "grid",
              tableWidth: usableW,
              ...
              columnStyles: {
                ...
                1: { cellWidth: 25 },
                2: { cellWidth: 15 },
                3: { cellWidth: usableW - 65 }, // Remaining width for reason
              },
              didDrawPage: (dataInfo: { pageNumber: number }) => {
