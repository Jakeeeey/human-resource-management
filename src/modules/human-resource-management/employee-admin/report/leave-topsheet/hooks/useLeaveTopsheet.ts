...
import { fetchLeaveTopsheet } from "../providers/fetchProvider";
import type { TopsheetUserSummary } from "../type";
import { toast } from "sonner";
import { format } from "date-fns";

export function useLeaveTopsheet() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await fetchLeaveTopsheet(startStr, endStr, departmentId);
      setData(response.data);
      if (response.departments) {
        setDepartments(response.departments);
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : "Failed to load leave topsheet";
