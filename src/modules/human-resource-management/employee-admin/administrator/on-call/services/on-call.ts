import { OnCallSchedule, OnCallList, EnrichedOnCallSchedule } from "../types/on-call.schema";
...
export const onCallService = {
        ...
        try {
            ...
            const enrichedSchedules = await Promise.all(
                schedules.map(async (schedule) => {
                    ...
                    const deptObj = schedule.department_id;
                    const encoderObj = schedule.encoder_id;

                    return {
                        ...schedule,
                        // Ensure IDs are strictly numbers/strings and names are strings
                        department_id: typeof deptObj === "object" ? (deptObj.department_id || deptObj.id) : deptObj,
                        department_name: typeof deptObj === "object" ? deptObj.department_name : "Unknown Department",
                        ...
                        last_edited_by: encoderObj ? {
                            user_fname: typeof encoderObj === "object" ? encoderObj.user_fname : "System",
                            user_lname: typeof encoderObj === "object" ? encoderObj.user_lname : "",
                            updated_at: schedule.updated_at || schedule.created_at || null,
                        } : undefined,
                        encoder_id: typeof encoderObj === "object" ? (encoderObj.user_id || encoderObj.id) : encoderObj,
    ...
    async updateSchedule(id: number, schedule: Partial<OnCallSchedule>, staffIds: number[]): Promise<void> {
        try {
            ...
            const { id: _id, created_at: _ca, ...dataToUpdate } = schedule as any;
            ...
            const currentIds = currentResult.data.map((a: any) => a.id);
