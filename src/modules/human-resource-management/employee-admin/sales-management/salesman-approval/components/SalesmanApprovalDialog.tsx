...
import { usePsgc } from "../../../sales-management/salesman-creation/hooks/usePsgc";
...
interface SalesmanApprovalDialogProps {
    open: boolean;
...
export function SalesmanApprovalDialog({
    open,
    onOpenChange,
    draft,
    users,
    ...
    useEffect(() => {
        if (open && draft) {
            form.reset({
                employee_id: draft.employee_id?.toString() || "",
                salesman_code: draft.salesman_code || "",
                salesman_name: draft.salesman_name || "",
                truck_plate: draft.truck_plate || "",
                user_email: draft.employee?.user_email || "",
                user_contact: draft.employee?.user_contact || "",
                user_province: draft.employee?.user_province || "",
                user_city: draft.employee?.user_city || "",
                user_brgy: draft.employee?.user_brgy || "",
                division_id: draft.division_id?.toString() || "",
                branch_code: draft.branch_code?.toString() || "",
                bad_branch_code: draft.bad_branch_code?.toString() || "",
                operation: draft.operation?.toString() || "",
                price_type_id: draft.price_type_id?.toString() || "",
                isActive: draft.isActive === 1,
                isInventory: draft.isInventory === 1,
                canCollect: draft.canCollect === 1,
                inventory_day: draft.inventory_day?.toString() || "",
            });
        }
    }, [open, draft, form]);
    ...
    const handleSubmit = async (data: SalesmanFormData) => {
        ...
        const toastId = toast.loading("Approving salesman draft...");
        try {
            ...
            toast.success("Salesman draft approved!", { id: toastId });
            onOpenChange(false);
            form.reset();
        } catch (error) {
            console.error("Error submitting salesman:", error);
            toast.error("Failed to approve salesman draft", {
                id: toastId,
                description:
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>
                        Approve Salesman Draft
                    </DialogTitle>
                    <DialogDescription>
                        Review and edit the draft information before approving it into an active salesman. Fields marked with * are required.
                    </DialogDescription>
                </DialogHeader>
