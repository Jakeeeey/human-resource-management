// T3 — plaintext converter (READ-ONLY copy of recruitment/mailing utils/
// mailText.ts, byte-identical logic): strips tags, maps links to
// `text (url)`. Used by dispatch for multipart/alternative. Pure +
// server-safe (string/regex ops only — no DOM). No-var: canvas HTML is
// final, so this runs verbatim on the compiled body.
 ...
 * @param html - Final compiled HTML (canvas export output).
