...
// T3 — mail transport factory (server-only — nodemailer + process.env, never
// imported client-side). READ-ONLY copy of recruitment/mailing providers/
// mailTransport.ts — logic byte-faithful, exports renamed ms*/Ms*.
//
...
// line passes through msRedact()/msLogRedacted() so secret VALUES never reach
// logs, outbox rows, or evidence files.
...
export const MS_PRIMARY_ENV_NAMES = [
    "MAIL_HOST",
    "MAIL_PORT",
    "MAIL_USER",
    "MAIL_PASS",
...
export const MS_FALLBACK_ENV_NAMES = [
...
export interface MsMailConfigStatus {
    /** True when every MS_PRIMARY_ENV_NAMES entry is non-empty. */
    configured: boolean;
...
export interface MsMailTransportResult {
 ...
 */
export function msRedact<T>(value: T): T {
    try {
        if (value instanceof Error) {
            const scrubbed = new Error(msScrubSecretsFromText(value.message));
            scrubbed.name = value.name;
            return scrubbed as T;
        }
        if (Array.isArray(value)) {
            return value.map((item) => msRedact(item)) as T;
        }
        if (value !== null && typeof value === "object") {
            const out: Record<string, unknown> = {};
            for (const [key, entry] of Object.entries(value as Record<string, unknown>)) {
                out[key] = SECRET_KEY_PATTERN.test(key) ? REDACTED : msRedact(entry);
            }
            return out as T;
        }
        return value;
 ...
 */
export function msScrubSecretsFromText(text: string): string {
 ...
 */
export function msLogRedacted(...args: unknown[]): void {
    console.error(...args.map((arg) => msRedact(arg)));
}
 ...
 */
export function getMsMailConfigStatus(): MsMailConfigStatus {
    const missing = missingNames(MS_PRIMARY_ENV_NAMES);
    const fallbackMissing = missingNames(MS_FALLBACK_ENV_NAMES);
 ...
 * Error carrying NAMES only — the caller (dispatch-service, T3) converts it
 * to a failed/skipped outbox outcome. Never sends mail itself.
 */
export async function getMsMailTransport(): Promise<MsMailTransportResult> {
    ...
    if (!primary) {
        const missing = missingNames(MS_PRIMARY_ENV_NAMES);
        msLogRedacted("[mail-transport] primary not configured, missing:", missing);
        throw new Error(
    ...
    try {
        ...
        msLogRedacted("[mail-transport] primary verify failed:", primaryError);
        ...
        try {
            ...
            await transporter.verify();
            msLogRedacted("[mail-transport] fallback verify succeeded (names only).");
            return { transporter, mode: "fallback", dryRun, ratePerMinute };
        } catch (fallbackError) {
            msLogRedacted("[mail-transport] fallback verify failed:", fallbackError);
            throw new Error(
