...
const BASE = "/api/hrm/mailing-studio/studio-templates";
...
export async function sendCompiledTest(
    ...
    const res = await fetch("/api/hrm/mailing-studio/studio-send/manual-send", {
        method: "POST",
...
export async function previewDesign(
    ...
    const res = await fetch("/api/hrm/mailing-studio/studio-templates/preview", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            design_json,
...
export const IMAGE_UPLOADS_ENDPOINT = "/api/hrm/mailing-studio/studio-templates/uploads";
