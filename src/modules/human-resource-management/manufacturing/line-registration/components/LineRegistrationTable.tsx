...
export function LineRegistrationTable({
    data,
    onEdit,
    onDelete,
    onManagePositions,
    onAdd,
    isLoading = false,
}: LineRegistrationTableProps) {
    const [sorting, setSorting] = React.useState<SortingState>([]);
    const [columnFilters, setColumnFilters] = React.useState<ColumnFiltersState>([]);

    const columns = React.useMemo(
        () => createColumns(onEdit, onDelete, onManagePositions),
        [onEdit, onDelete, onManagePositions]
    );
                    ...
                    {Array.from({ length: 5 }).map((_, i) => (
                        ...
                        >
                            ...
                            <div className="flex gap-1">
                                <div className="h-8 w-8 rounded-xl bg-muted/10 animate-pulse" />
                                <div className="h-8 w-8 rounded-xl bg-muted/10 animate-pulse" />
                                <div className="h-8 w-8 rounded-xl bg-muted/10 animate-pulse" />
                            </div>
                        </div>
        ...
        <div className="space-y-4">
                        ...
                        {table.getRowModel().rows?.length ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={columns.length} className="h-24 text-center">
                                    No production lines registered.
                                </TableCell>
                            </TableRow>
                        )}
                    </TableBody>
            ...
            <div className="flex items-center justify-end space-x-2">
                <div className="flex-1 text-sm text-muted-foreground">
                    {table.getFilteredRowModel().rows.length} production line(s) registered
                </div>
