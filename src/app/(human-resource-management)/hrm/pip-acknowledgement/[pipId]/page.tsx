...
export const metadata = {
  title: "PIP Details | HRM",
  description: "Review and acknowledge a single Performance Improvement Plan.",
};
...
export default async function Page({
  ...
  const { pipId: rawPipId } = await params;
  const pipId = Number.parseInt(rawPipId, 10);
  if (!Number.isInteger(pipId) || pipId <= 0) notFound();
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
          ...
          <div className="min-w-0 overflow-hidden">
            <Breadcrumb>
              <BreadcrumbList className="min-w-0 overflow-hidden">
                ...
                <BreadcrumbItem className="hidden md:block shrink-0">
                  <BreadcrumbLink href="/hrm/pip-acknowledgement">PIP Acknowledgement</BreadcrumbLink>
                </BreadcrumbItem>
                <BreadcrumbSeparator className="hidden md:block shrink-0" />
      ...
      <main className="min-h-0 min-w-0 flex-1 overflow-y-auto overflow-x-hidden p-2 sm:p-4">
        <PipAcknowledgeView pipId={pipId} />
      </main>
    </div>
  );
}
