...
async function proxy() {
  ...
  const upstreamUrl = `${UPSTREAM_BASE.replace(/\/+$/, "")}/items/subsystems?fields=*,modules.*,modules.subModules.*&limit=-1`;
  ...
  try {
    ...
    if (!res.ok) {
        ...
        console.error(`[Dashboard - Subsystems Proxy] Upstream Error (${res.status}):`, errorBody);
    ...
    console.error(`[Dashboard - Subsystems Proxy] Fetch error:`, message);
