...
export async function GET() {
    ...
    if (!baseUrl) {
        console.error("[Dashboard - User Profile Proxy] NEXT_PUBLIC_API_BASE_URL missing.");
    ...
    try {
        ...
        if (!subRes.ok || !modRes.ok) {
            console.error(`[Dashboard - User Profile Proxy] Permission Fetch Failed: Subs=${subRes.status}, Mods=${modRes.status}`);
        ...
        const subsystemIds = (subData.data || []).map((row: { subsystem_id?: { id?: number } }) => row.subsystem_id?.id).filter(Boolean) as number[];
        const moduleIds = (modData.data || []).map((row: { module_id?: { id?: number } }) => row.module_id?.id).filter(Boolean) as number[];
        ...
        console.error("[Dashboard - User Profile Proxy] Fatal Error:", error);
