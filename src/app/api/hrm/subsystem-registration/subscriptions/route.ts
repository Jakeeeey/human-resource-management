...
async function proxy(req: NextRequest) {
  if (!UPSTREAM_BASE) {
    console.error("[Subsystem Reg - Subscriptions Proxy] NEXT_PUBLIC_API_BASE_URL missing.");
  ...
  // Construct upstream URL (using singular table name 'subscription' to match MySQL DDL)
  let upstreamUrl = `${UPSTREAM_BASE.replace(/\/+$/, "")}/items/subscription`;
  ...
  try {
    ...
    if (!res.ok) {
        ...
        console.error(`[Subsystem Reg - Subscriptions Proxy] Upstream Error (${res.status}):`, errorBody);
    ...
    console.error(`[Subsystem Reg - Subscriptions Proxy] Connection Error:`, message);
