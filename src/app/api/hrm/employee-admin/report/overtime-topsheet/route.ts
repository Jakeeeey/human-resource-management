...
export async function GET(req: NextRequest) {
  try {
    ...
    // Override requested department if they are not allowed to view "All" or other departments
    ...
    // Build query for overtime requests
    let overtimeUrl = `/items/overtime_request?limit=-1&sort=-request_date&fields=*&filter[status][_eq]=approved`;

    if (targetDepartmentId) {
      overtimeUrl += `&filter[department_id][_eq]=${targetDepartmentId}`;
    }

    if (startDate) {
      overtimeUrl += `&filter[request_date][_gte]=${startDate}`;
    }

    if (endDate) {
      overtimeUrl += `&filter[request_date][_lte]=${endDate}`;
    }

    const overtimeResponse = await directusFetch(overtimeUrl);
    const requests: OvertimeRequest[] = overtimeResponse.data || [];
    ...
    if (userIds.length > 0) {
      // Chunk user fetches if too many to avoid URL length limit, but usually fine for typical payload
    ...
    for (const req of requests) {
      ...
      if (group) {
        group.total_duration_minutes += req.duration_minutes || 0;
    ...
    console.error("GET overtime topsheet error:", error);
    return NextResponse.json(
      { error: "Failed to fetch overtime topsheet data" },
      { status: 500 }
    );
  }
}
