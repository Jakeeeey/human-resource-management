...
export async function GET(req: NextRequest) {
  try {
    ...
    // Build query for undertime requests
    let undertimeUrl = `/items/undertime_request?limit=-1&sort=-request_date&fields=*&filter[status][_eq]=approved`;

    if (targetDepartmentId) {
      undertimeUrl += `&filter[department_id][_eq]=${targetDepartmentId}`;
    }

    if (startDate) {
      undertimeUrl += `&filter[request_date][_gte]=${startDate}`;
    }

    if (endDate) {
      undertimeUrl += `&filter[request_date][_lte]=${endDate}`;
    }

    const undertimeResponse = await directusFetch(undertimeUrl);
    const requests: UndertimeRequest[] = undertimeResponse.data || [];
    ...
    console.error("GET undertime topsheet error:", error);
    return NextResponse.json(
      { error: "Failed to fetch undertime topsheet data" },
      { status: 500 }
    );
  }
}
