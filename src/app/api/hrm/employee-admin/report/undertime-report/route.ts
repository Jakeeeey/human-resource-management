...
export async function GET(req: NextRequest) {
  try {
    ...
    const undertimeUrl  = `/items/undertime_request?limit=${pageSize}&page=${page}&sort=-filed_at&fields=*&meta=filter_count${filterQuery}`;

    const undertimeResponse = await directusFetch(undertimeUrl);
    const requests   = undertimeResponse.data || [];
    const totalCount = undertimeResponse.meta?.filter_count ?? requests.length;
    ...
    // ── Enrich requests ──────────────────────────────────────────────────────
    const enrichedRequests = requests.map((req: UndertimeRequest) => {
    ...
    return NextResponse.json({
      currentUser,
      departments,
      undertimeRequests: enrichedRequests,
      pagination: {
        currentPage: page,
        pageSize,
    ...
    console.error("GET undertime report error:", error);
    return NextResponse.json(
      { error: "Failed to fetch undertime report data" },
      { status: 500 }
    );
  }
}
