...
async function proxy(req: NextRequest) {
    ...
    let upstreamUrl = `${UPSTREAM_BASE.replace(/\/+$/, "")}/items/manu_hr_schedule_positions`;
    if (id) upstreamUrl += `/${id}`;

    // Forward all query parameters from the client to upstream
    const queryParams = new URLSearchParams(searchParams);
    queryParams.delete("id"); // Remove proxy-specific param
    ...
    try {
        ...
        if (!res.ok) {
            ...
            console.error(`[Manufacturing - Schedule Positions Proxy] Upstream Error (${res.status}):`, errorBody);
        ...
        console.error(`[Manufacturing - Schedule Positions Proxy] Connection Error:`, message);
