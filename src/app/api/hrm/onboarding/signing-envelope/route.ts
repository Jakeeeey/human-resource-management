...
// GET /api/hrm/onboarding/signing-envelope — list the applicant-scoped
// aggregate rows, optionally scoped with `?applicant_id=` / `?status=`.
// POST /api/hrm/onboarding/signing-envelope — create one envelope;
...
export async function GET(req: NextRequest) {
  try {
    ...
    const query = SigningEnvelopeListQuerySchema.safeParse(params);
    if (!query.success) {
    ...
    const result = (await dFetch(`/items/signing_envelope${suffix}`)) as {
      data?: unknown[];
      errors?: unknown;
    };
    if (result?.errors) {
      console.error(
        "[onboarding-signing-envelope] list failed:",
    ...
    for (const row of rows) {
      const rowParsed = SigningEnvelopeSchema.safeParse(row);
    ...
    console.error("[onboarding-signing-envelope] list error:", error);
    return serverError();
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (created?.errors || !created?.data) {
      console.error(
        "[onboarding-signing-envelope] create failed:",
    ...
    const parsed = SigningEnvelopeSchema.safeParse(created.data);
    if (!parsed.success) {
      console.error(
        "[onboarding-signing-envelope] created row contract mismatch:",
    ...
    console.error("[onboarding-signing-envelope] create error:", error);
    return serverError();
  }
}
