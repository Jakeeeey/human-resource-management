...
// POST /api/hrm/onboarding/job-offer/upload — offer PDF intake. PDF-only
// (415 otherwise, mime + magic bytes — a client `accept` hint alone is never
// trusted), 10MB cap (413), filed into the `offer_files` Directus folder
// (created on first use — the get-or-create flow mirrors
// `paperwork-templates/upload`, as does the metadata-before-binary
// ordering), returning the file UUID the offer record persists as
// `pdf_file`.
const MAX_FILE_SIZE = 10 * 1024 * 1024;
const PDF_MIME = "application/pdf";
const FOLDER_NAME = "offer_files";
const PDF_MAGIC = "%PDF-";
  ...
  try {
    ...
    try {
      ...
      if (!folderId) {
        ...
        if (!folderId) {
          console.error(
            `[offer-upload] Could not create folder "${FOLDER_NAME}":`,
            created
          );
        }
      }
    } catch (err) {
      console.error(`[offer-upload] Error resolving folder "${FOLDER_NAME}":`, err);
    }
    ...
    if (folderId) {
      ...
      console.warn("[offer-upload] No valid folder ID resolved; uploading without folder");
    }
    ...
    if (!response.ok) {
      console.error("[offer-upload] Directus upload error:", result);
      return fail(
    ...
    console.error("[offer-upload] upload error:", error);
    return fail(
