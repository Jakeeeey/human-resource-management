...
import { UpdateEquipmentItemBodySchema } from "@/modules/human-resource-management/onboarding/requirements/types/requirements-api.schema";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

// PATCH  /api/hrm/onboarding/requirements/equipment/[id] — partial update
//        (label / issuer / is_required / is_active / sort_order).
// DELETE .../[id] — SOFT delete (`is_active=0`); the row stays readable.
//        `?hard=1` requests a true DELETE after the task-reference guard
//        (equipment items carry no `onboarding_task` FK, so the guard passes).
...
export async function PATCH(
  ...
  try {
    ...
    const validation = UpdateEquipmentItemBodySchema.safeParse(body);
    if (!validation.success) {
      return validationFailed(validation.error.flatten().fieldErrors);
    }

    const current = (await listEquipmentItemRows()).find((row) => row.id === id);
    if (!current) return requirementsNotFound("Equipment item not found");

    const patch: Record<string, unknown> = {};
    if (validation.data.label !== undefined) patch.label = validation.data.label;
    if (validation.data.issuer !== undefined) {
      patch.issuer = validation.data.issuer;
    }
    ...
    console.error("[requirements-equipment] update error:", error);
...
export async function DELETE(
  ...
  try {
    ...
    if (req.nextUrl.searchParams.get("hard") === "1") {
      await hardDeleteItem("onboarding_equipment_item", id);
      return NextResponse.json({ success: true, data: { id, deleted: true } });
    }

    const soft = await softDeleteEquipmentItemRow(id, session.actorId);
    return NextResponse.json({ success: true, data: soft });
  } catch (error) {
    console.error("[requirements-equipment] delete error:", error);
    return mapRequirementsFailure(error);
  }
}
