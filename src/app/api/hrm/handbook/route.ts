...
export async function GET() {
    try {
        const data = await handbookService.fetchAll();
...
export async function POST(req: NextRequest) {
    try {
        ...
        // Inject created_by
        if (userId) {
            body.created_by = typeof userId === "string" ? parseInt(userId) : userId;
        }

        const validated = handbookSchema.parse(body);
        const data = await handbookService.create(validated);
        return NextResponse.json(data, { status: 201 });
    } catch (e: unknown) {
        console.error("Error in POST /api/hrm/handbook:", e);
